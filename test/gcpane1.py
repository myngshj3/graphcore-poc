...
class ShapeHolder:
    ...
    def blink(self):
        ...
        for k in self.shapes.keys():
            ...
            # dist = random.random() * 20.0
            # dx = dist * math.cos(angle)
            # dy = dist * math.sin(angle)
            # self.move_shape_by(k, dx, dy)
            k = 30
            m = 2.5
            oa = math.sqrt(k/m)
            ob = math.sqrt(3*k/m)
            A1, A2, B1, B2 = 1, 3, 0.6, 1.5
            global x1, x2, t
            t += 0.01
            x1 += A1*math.sin(oa*t) + A2*math.cos(oa*t) + B1*math.sin(ob*t) + B2*math.cos(ob*t)
            x2 += A1*math.sin(oa*t) + A2*math.cos(oa*t) - B1*math.sin(ob*t) - B2*math.cos(ob*t)
            obj: QGraphicsItem = self.shapes[3][self.GUI_OBJECT]
            x = self.shapes[3][self.X]
            self.move_shape_by(3, x1-x, 0)
            obj: QGraphicsItem = self.shapes[4][self.GUI_OBJECT]
            x = self.shapes[4][self.X]
            self.move_shape_by(4, x2-x, 0)
...
class GCPane:
    ...
    def setupGui(self, args):
        ...
        # self.shape_holder.new_box(-200, 0, 10, 10)
        # self.shape_holder.new_circle(0, 0, 100)
        # self.shape_holder.new_circle(-100, -50, 100)
        # self.shape_holder.new_edge(0, 1)
        # self.shape_holder.new_edge(0, 2)
        # self.shape_holder.new_edge(0, 3)
        # self.shape_holder.new_edge(1, 2)
        # self.shape_holder.new_edge(1, 3)
        # self.shape_holder.new_edge(2, 3)
        sid = self.shape_holder.new_box(-220, -30, 40, 60)  # 0
        self.shape_holder.set_shape_name(sid, "Lwall")
        sid = self.shape_holder.new_box(220, -30, 40, 60)  # 1
        self.shape_holder.set_shape_name(sid, "Rwall")
        sid = self.shape_holder.new_box(0, -10, 400, 20)  # 2
        self.shape_holder.set_shape_name(sid, "Floor")
        global x1, x2
        sid = self.shape_holder.new_circle(x1, -30, 30)  # 3
        self.shape_holder.set_shape_name(sid, "B1")
        sid = self.shape_holder.new_circle(x2, -30, 30)  # 4
        self.shape_holder.set_shape_name(sid, "B2")
        # connect with springs
        self.shape_holder.new_edge(0, 3)
        self.shape_holder.new_edge(3, 4)
        self.shape_holder.new_edge(4, 1)
